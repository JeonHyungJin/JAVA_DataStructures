...
public class AvlTree implements Tree {
	...
	private Node delete(Node node, int data) {
		...
		if (data < node.getData()) { // data smaller than given node's data ->
										// go to the left recursively
			node.setLeftNode(delete(node.getLeftNode(), data));
		} else if (data > node.getData()) { // data greater than given node's
											// data -> go to the right
											// recursively
