...
public class RedBlackTree {
	...
	private void fixViolations(Node node) {
		...
		while (node != root && node.getColor() != NodeColor.BLACK && node.getParent().getColor() == NodeColor.RED) {
			...
			if (parentNode == grandParentNode.getLeftChild()) {
				...
				// given node x's parent is a left child + uncle is red --> only
				// recoloring
				if (uncle != null && uncle.getColor() == NodeColor.RED) {
					...
					node = grandParentNode; // this will be the x after the
	...
	private void rightRotate(Node node) { // az input a beszurt node
											// grandparentje
	...
	private void leftRotate(Node node) { // az input a beszurt node
											// grandparentje
