package GenericHashMapLinearProbing;
...
public class HashTable<Key, Value> {
	...
	// have to define a different constructor because when we resize the table
	// we use it !!!
	...
	public Value get(Key key) {
		...
		// we have to consider the items right after each other because the item
		// could have been shifted down
	...
	public void put(Key key, Value value) {
		...
		// load balance is 0.75: so when the table is 75% full we resize it -->
		// double its size
		// why? when it is nearly empty --> we waste a lot of memory for no
		// reason
		// when it is nearly full --> there will be lots of collisions --> O(1)
		// will reduce to O(N) or something like that
		if (numOfItems >= capacity * 0.75)
			resize(2 * capacity);

		int index = hash(key);

		// maybe there is a collision and there is already an item inserted to
		// that given index, so we have to
		...
		// we have managed to found the array index where we can insert the
		// value --> so update accordingly !!!
		keys[index] = key;
		values[index] = value;
		numOfItems++;
	}
	...
	public void remove(Key key) {
		...
		// we have to reconstruct the table starting from the item deleted:
		...
		while (keys[index] != null) {
			...
			// we have to decrement the size, because with the put() method it
	...
	private void resize(int newCapacity) {
		...
		// have to rehash the table entries because the hashfunction relies
		// heavily on the
		// size: ~ key.hashCode() % sizeOfTable !!!
		// So it is a O(N) operation --> we should make as few resize operation
		// as possible !!!
